...
## How region grouping works

`state_regions.json` maps each of the 36 states/UTs to one of seven regions, and the site groups the chart by region by default (with a toggle back to a plain A–Z list, since "look up my own state" is still the primary use case). The regions follow India's official Zonal Council structure (Northern, Central, Eastern, Western, Southern, North-Eastern Zones — set up under the States Reorganisation Act, 1956 and the Punjab Reorganisation Act, 1966 framework) rather than an invented grouping, since that's a defensible, citable classification rather than a subjective one.

Two judgment calls worth flagging: the Andaman & Nicobar Islands and Lakshadweep don't belong to any Zonal Council (they're island territories outside that mainland structure), so they're grouped here as a separate "Island Territories" region rather than folded arbitrarily into whichever mainland zone is geographically nearest. And Sikkim is included under North-Eastern Zone for this dataset's purposes, matching its de facto administration under the North Eastern Council, even though it joined that council later (1975) than the original states.

This grouping is also just a genuinely useful lens on the data itself: a lot of the unit families in this dataset visibly cluster by region rather than by state — the Bigha/Biswa system spans most of the Northern and Central Zone, Kanal/Marla is a Northern-Zone hill-and-Punjab-belt pattern, Guntha shows up across the Western and Southern Zones, and Cent/Ground/Kuzhi is a Southern-Zone cluster. Grouping by region makes that visible in a way an alphabetical list doesn't.
...
## Third pass: targeted follow-up on remaining unverified entries

Went looking specifically for an India Code circular for each remaining unverified state, the way Tripura had one. None turned up — Tripura appears to be a one-off rather than a pattern, which itself is a useful (negative) result: further generic searching on the rest is unlikely to convert them from unverified to sourced. One real catch came out of this pass, though:

**Bug found and fixed: Himachal Pradesh and Uttarakhand's "Killa" was in the wrong unit.** Cross-checking against Wikipedia's well-cited *"Measurement of land in Punjab"* article (which sources the 1883–84 Punjab District Gazetteers and the Haryana Jamabandi portal) confirmed that 1 Killa = 1 Acre = 4,840 **square yards** = 43,560 **square feet**. The dataset had `Killa: 4840`, which is the correct number but the wrong unit — every other value in this file is square feet, so 4,840 was silently off by a factor of 9 (reporting roughly 1/9 of an acre instead of a full acre). This looks like a square-yard figure that got copied in without converting, somewhere before this project's edits began. Corrected to `43560` for both states. [Measurement of land in Punjab — Wikipedia](https://en.wikipedia.org/wiki/Measurement_of_land_in_Punjab)

**Confirms Punjab/Haryana/UP's Sarsahi (30.25) is correct**, and explains why Biswansi/Biswa Kachha are hard to pin down: the same Wikipedia article documents at least four different historical Karam-lengths (66", 60", 57.5", 54") across different Punjab districts, each producing a different Sarsahi/Marla/Kanal value. The existing `Biswansi: 22` figure lands closest to the 57.5" Karam variant (Hoshiarpur/Jalandhar area, ≈22.96 sq ft), which is plausible but not confirmed as *the* figure for Haryana/Punjab/UP specifically — this is a genuinely regional variation, not a data error, so left as-is.

**Lakshadweep — named the actual regulation, still no numeric confirmation.** The governing law is *The Laccadive, Minicoy and Amindivi Islands Land Revenue and Tenancy Regulation* (amended 2023). No specific mention of Cent or any local unit turned up in what's searchable, so it stays flagged.

**Arunachal Pradesh, Jharkhand, Rajasthan Biswa, Manipur** — no better sources than what's already documented above. These remain flagged unverified; further progress here most likely requires a primary district-level Settlement Report or Gazetteer that isn't indexed in a generally web-searchable form, which is a real ceiling on how far this kind of research can go without physical archive access.
